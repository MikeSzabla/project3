...
@pytest.fixture
def correct_test_dataframe():
    correct_test_dataframe = pd.DataFrame({'value_a': [1.0], 'value_b': [1.0],
                                           'operation': ['subtraction'],
                                           'input_file': ['input_test.csv'],
                                           'record_number': [1.0]})
    return correct_test_dataframe


@pytest.fixture(name="input_data_fixture")
def input_data_fixture(io_test_dir):
...
def test_get_list_of_input_paths(io_test_dir, input_data_fixture):
...
def test_set_list_of_input_paths(input_data_fixture):
...
def test_get_input_dataframe(input_data_fixture):
    """tests the get_input_dataframe method"""
    df = InputData.get_input_dataframe()
    assert isinstance(df, pd.core.frame.DataFrame)


def test_append_dataframe(input_data_fixture, correct_test_dataframe):
    """tests the append_dataframe method"""
    InputData.append_dataframe(correct_test_dataframe)
    assert InputData.get_input_dataframe().equals(pd.concat([correct_test_dataframe, correct_test_dataframe], ignore_index=True))


def test_populate_dataframe(input_data_fixture, correct_test_dataframe):
    """tests the populate_dataframe method"""
    # populate_dataframe is called in the constructor so the results are being testing
    assert InputData.get_input_dataframe().equals(correct_test_dataframe)
